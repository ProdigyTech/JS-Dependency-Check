      ...
      template: e.length
        ? `\n                    <h2>${n}</h2>\n                <table id="result-table-${n} style="width:100%">\n                    <thead>\n                        <tr>\n                        <td>Package</td>\n                        <td>Current Version</td>\n                        <td>Current Release Date</td>\n                        <td>Latest Version</td>\n                        <td>Latest Version Release Date</td>\n                        <td>Status</td>\n                        <td> Upgrade Type </td>\n                        <td> Link to package in registry </td>\n                        <td> Link to package on NPM </td>\n                        </tr>\n                    </thead>\n                    <tbody>\n                       ${e
            .map(({ package: e }) => {
              ...
              "OUTDATED" === c && r++;
              const p = new Date(d.releaseDate).toLocaleDateString(),
                l = new Date(o.releaseDate).toLocaleDateString();
              return `<tr>\n                        <td>${n}</td>\n                        <td> ${
                d.version
              } </td>\n                        <td>${p}</td>\n                        <td>${
                o.version
                ...
                if ("N/A" === e) return "background-color:green";
                switch (e) {
                  case "PATCH":
...
const g = await (async () => {
    ...
    return {
      ...
      peerDependencies: o(r.peerDependencies) || [],
      devDependencies: o(r.devDependencies) || [],
    };
  })(),
  { peerDependencies: h, dependencies: m, devDependencies: v, repoInfo: D } = g,
  b = await (async ({
    peerDependencies: e = [],
    devDependencies: n = [],
    dependencies: t = [],
  ...
  })({ peerDependencies: h, dependencies: m, devDependencies: v }),
  y = (({
    peerDependenciesResult: e,
    devDependenciesResult: n,
    dependenciesResult: t,
  }) => {
    const { template: r, outdated_counter: a } = d(t, "Dependencies"),
      { template: s, outdated_counter: o } = d(n, "Dev Dependencies"),
      { template: i, outdated_counter: c } = d(e, "Peer Dependencies");
